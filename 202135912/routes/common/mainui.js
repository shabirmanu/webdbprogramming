...
let    htmlstream = '';
     ...
     if (req.session.auth && req.session.admin) {  // 만약, 관리자가 로그인했다면
       ...
       htmlstream = htmlstream + fs.readFileSync(__dirname + '/../../views/admin/admincontent.ejs','utf8');
     } else {
       htmlstream = htmlstream + fs.readFileSync(__dirname + '/../../views/common/navbar.ejs','utf8');  // 일반사용자메뉴
       htmlstream = htmlstream + fs.readFileSync(__dirname + '/../../views/common/content.ejs','utf8'); // Content
     }
